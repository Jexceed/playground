# Implementation Plan: Three-View Block Logic Quality

**Branch**: `007-three-view-quality` | **Date**: 2026-07-04 | **Spec**: `specs/007-three-view-quality/spec.md`

**Input**: Feature specification from `specs/007-three-view-quality/spec.md`

## Summary

Continue logic-house quality work by tightening audit checks and rewriting the
`logic-three-view-blocks` cluster. The slice covers three related strategies:
top view counts non-zero positions, front view reads column maximums, and left
view reads row maximums. Local voice lines and maintained docs stay aligned.
...
model, existing grid rendering, existing Edge voice generation scripts
...
manifest generation/validation, `pnpm build`, targeted three-view review, and
`git diff --check`
...
avoid new images because existing grid surfaces support this cluster
...
- Child-centered learning integrity: pass. The feature focuses on concrete,
  explainable viewpoint strategies and avoids trick wording.
...
```text
scripts/
└── audit-curriculum.mjs          # add three-view quality findings

src/
└── data/
    └── games.ts                  # rewrite logic-three-view-blocks cluster
...
See `data-model.md`, `contracts/three-view-audit-report.md`, and
`quickstart.md`.

## Post-Design Constitution Check

- Child-centered learning integrity: pass. The design makes top/front/left view
  strategies concrete and parent-explainable.
- Spec-driven traceability: pass. Requirements map to task phases and audit
  findings.
- Auditable local assets: pass. Existing grid surfaces remain the visual source;
