# Implementation Plan: Address-Map Logic Quality

**Branch**: `009-address-map-quality` | **Date**: 2026-07-04 | **Spec**: `specs/009-address-map-quality/spec.md`

**Input**: Feature specification from `specs/009-address-map-quality/spec.md`

## Summary

Continue logic-house quality work by tightening audit checks and rewriting the
`logic-address-map` cluster. The slice verifies answers from visible row-column
grids, then improves feedback so address-to-object and object-to-address rounds
name the row, column, crossing cell, and final answer. Local voice lines and
...
manifest generation/validation, `pnpm build`, targeted address-map review, and
`git diff --check`
...
avoid new images because the existing treasure-map scene plus grid supports this
cluster
...
- Child-centered learning integrity: pass. The feature focuses on visible,
  pointable row-column intersections and avoids guessing.
...
```text
scripts/
└── audit-curriculum.mjs          # add address-map quality findings

src/
└── data/
    └── games.ts                  # rewrite logic-address-map cluster
...
See `data-model.md`, `contracts/address-map-audit-report.md`, and
`quickstart.md`.

## Post-Design Constitution Check

- Child-centered learning integrity: pass. The design makes row-column location
  concrete, ordered, and explainable.
- Spec-driven traceability: pass. Requirements map to task phases and audit
  findings.
- Auditable local assets: pass. Existing grid/scene surfaces remain the visual
