# Implementation Plan: Route-Step Logic Quality

**Branch**: `008-route-steps-quality` | **Date**: 2026-07-04 | **Spec**: `specs/008-route-steps-quality/spec.md`

**Input**: Feature specification from `specs/008-route-steps-quality/spec.md`

## Summary

Continue logic-house quality work by tightening audit checks and rewriting the
`logic-route-steps` cluster. The slice verifies route answers from visible grid
movement, then improves feedback so one-step and two-step routes name the start,
ordered moves, intermediate cells, and final destination. Local voice lines and
...
manifest generation/validation, `pnpm build`, targeted route-step review, and
`git diff --check`
...
avoid new images because existing grid and scene surfaces support this cluster
...
- Child-centered learning integrity: pass. The feature focuses on visible,
  pointable route steps and avoids guessing.
...
```text
scripts/
└── audit-curriculum.mjs          # add route-step quality findings

src/
└── data/
    └── games.ts                  # rewrite logic-route-steps cluster
...
See `data-model.md`, `contracts/route-steps-audit-report.md`, and
`quickstart.md`.

## Post-Design Constitution Check

- Child-centered learning integrity: pass. The design makes route movement
