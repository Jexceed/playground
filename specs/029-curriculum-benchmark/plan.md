...
在一个 React/Tauri 应用中设置“启蒙”和“探索”两个部分。启蒙承接当前40游戏489轮，探索集中承接上实资料对标内容；每部分内按主题、题组组织，保留各自进度与续玩。不建立年龄版本分支，不使用V1/V2产品标签。以用户提供的本地资料为研究输入，建立来源—能力题族—活动—验证的链路，共用多选、槽位回填、记忆阶段和约束操作引擎。

## 2026-09-22 入口修正增量

- `src/curriculum/catalog.ts` 显式注册启蒙/探索及其题组，平铺目录继续服务审计、语音和出口，应用列表只读取当前部分。归属与交互类型解耦。
- `src/App.tsx` 在主题之上增加固定双入口，并显示当前部分名称、题量和完成题数。只列有内容的主题；切换恢复各部分上次位置，成长标签和清空操作只作用于当前部分。
- `src/services/curriculum-navigation.ts` 新增版本化分区续玩键，以稳定题目ID存储每部分位置；首次从旧目录/旧题位置兼容初始化，保留旧记录，损坏和未来版本不覆盖。
