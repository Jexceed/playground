...
- [x] T117 CRITICAL / F1 在 `src/curriculum/exploration/graphic.ts` 重建G15折纸打孔题的展开纸、折向、折起轮廓和有效打孔区，使用独立折叠/镜像推演核对；若题意改变需更新revision并保留旧事实。per Constitution I、FR-009、FR-014、SC-005 (contradicts)
- [x] T118 HIGH / F2 在 `math.ts`、`logic.ts`、`graphic.ts` 修正N13库存、L05最短路线、G04双标记的无效条件：必须存在总额正确但超库存、更长但其余条件合法、红标记正确但蓝标记错误的候选；独立枚举并保留合法多解，记录内容revision。per FR-005、FR-009、FR-022、US3/AC3 (partial)
- [x] T119 HIGH / F3 在 `graphic.ts` 与活动/交互模型中使G07金字塔中间层可填写、可回看、可逐层核验；保留顶层检查，区分中间推理与仅猜顶层。per FR-005、FR-008、FR-020、SC-002 (partial)
...
- [x] T128 HIGH / F12 逐族重评 `helpers.ts` 生成的难度元数据，移除reading/motor恒为1、rules/steps直接等于stage的默认校准假象；独立描述前置识读、操作负荷、规则/步骤、表征和支持，校核各阶有效变式并为T112亲子试玩准备观察表。per FR-005、FR-010、SC-002、SC-006 (partial)
...
### 034工程修复跟进（2026-09-25）

T117/T118/T119/T128已完成当前实现范围的独立设计、内容与引擎验证；24项提升revision，旧事实保留。T123的G09部分完成小网格3/6/9计数与标记工具，其他图形机制不据此关闭。当前Mac浏览器初始621项与代表性操作状态已检查；最新标准.app的15项代表性原生验收和重开恢复已于2026-09-25完成，见034验证记录。T130仍保留其它设备/窗口边界；T112真实儿童校准、T127来源任务映射和T129完整人工听审继续未完成。
