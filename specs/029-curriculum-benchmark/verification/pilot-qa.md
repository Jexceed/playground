...
电脑控制工具返回Mac已锁屏，已请求用户手动解锁。以下项目尚未验证，不以浏览器/构建结果代替：

- 打开更新后的真实Mac应用，操作新题、听题、退出重开及确认持久化。
- 更窄窗口和触屏布局检查。
- 在界面中等待记忆时间自动结束、切换前后台中断与重看的专项检查。纯状态转换已通过自动化测试，完整UI专项仍待补。
- 6–8岁亲子难度校准属于后续内容建设，不是本批已完成结果。
