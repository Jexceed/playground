...
- [ ] T007 手动解锁后完成余下原生输入检查，收尾035～038待办；开发检查点同步不等于这些检查已完成。
...
原生验收进展：新包启动、E03三图默认画面、不同名称等大、说明框外和整页操作可见已通过原生AX/截图确认；T009的原生输入及四图状态仍待解锁。按用户已有授权同步当前已确认显示修复的dev检查点，同时保留未覆盖项。
