...
const {explorationDrawings,soundStimuli}=await loadTypeScriptModule('src/curriculum/exploration/index.ts');
const directory='public/images/items/exploration';mkdirSync(join(directory,'source'),{recursive:true});
const esc=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
function object(o){const stroke='stroke="#344555" stroke-width="2.2" stroke-linejoin="round"';switch(o.kind){case'circle':return `<circle cx="${o.x}" cy="${o.y}" r="${o.r}" fill="${o.fill}" ${stroke}/>`;case'rect':return `<rect x="${o.x}" y="${o.y}" width="${o.w}" height="${o.h}" fill="${o.fill}" ${stroke} ${o.dash?'stroke-dasharray="8 6"':''}/>`;case'line':return `<line x1="${o.x}" y1="${o.y}" x2="${o.x2}" y2="${o.y2}" ${stroke} ${o.dash?'stroke-dasharray="8 6"':''}/>`;case'polygon':return `<polygon points="${o.points.map(p=>p.join(',')).join(' ')}" fill="${o.fill}" ${stroke}/>`;case'text':return `<text x="${o.x}" y="${o.y}" font-family="PingFang SC,Heiti SC,sans-serif" font-size="${o.size??28}" fill="${o.color??'#253243'}" text-anchor="middle">${esc(o.text)}</text>`;default:throw Error(o.kind);}}
for(const [id,drawing] of Object.entries(explorationDrawings)){
 ...
 if(!existsSync(join(directory,id+'.png')))await sharp(Buffer.from(svg)).png().toFile(join(directory,id+'.png'));
}
...
console.log(JSON.stringify({diagrams:Object.keys(explorationDrawings).length,sounds:Object.keys(soundStimuli).length}));
