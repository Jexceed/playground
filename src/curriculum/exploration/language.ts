...
export const languageSets = [
    ...
    set('E03', '故事接起来', nine((i, s, v) => { const stories = [['天上积起乌云', '开始下雨', '地面留下水洼', '小朋友穿雨靴出门'], ['小熊准备木块', '小熊搭起高塔', '球碰倒了高塔', '小熊换宽底座重搭'], ['小猫收到种子', '把种子种进土里', '按记录照顾小苗', '小猫观察新叶片']]; if (s === 2)
        return parent('E03', i, '先讲清故事，再想一个不同的结局。', ['屏幕上的四张故事图'], [...stories[v].map((t, k) => `第${k + 1}张：${t}。`), '按原先顺序复述一次。', '改变最后一件事，再解释为什么也说得通。'], ['事件先后清楚', '能用因为或所以连接理由', '不同结局和前面的事情有关']); const seq = stories[v].slice(0, 3 + s); return ordered('E03', i, '按这个故事的先后，把事件排好。', seq.map((t, k) => card(`e${k}`, t)), seq.map((_, k) => `e${k}`), '按题目给定的事件关系连接，再用自己的话讲出来。', seq.slice(1).map((t, k) => `${seq[k]}以后，才${t}。`)); })),
    set('E04', '我的解释和办法', three(i => {
        const a = parent('E04', i, ['给图卡编故事', '比较两个办法', '证据够不够'][i],
            i === 0 ? ['屏幕上的四张图卡'] : i === 1 ? ['积木', '小玩具', '纸条当小河'] : ['屏幕上的房间图'],
            i === 0 ? ['从四张图卡中选三张，说出故事里的角色和地点。', '讲一件把选中的三张图卡连起来的事。', '用剩下的一张替换其中一张，再调整故事使它仍然连贯。']
                : i === 1 ? ['把纸条当小河，用积木想两种让玩具过河的办法。', '分别说需要哪些材料，哪里可能失败。', '选一种试做，再用观察到的结果解释选择。']
                : ['先看房间图。家长说“地上有水，所以一定是小狗打翻了杯子”。', '指出哪些是图里看到的，哪些只是猜测；靠近水杯也不能证明是谁打翻的。', '提出另一种可能，并说还需要看到什么经过或线索。'],
            ['观点能和线索对应', '能说明一个理由', '能接受或提出另一种合理解释']);
        a.revision = 2; return a;
    })),
