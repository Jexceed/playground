...
- Review remaining text-only `VisualToken` fallbacks and decide which need
  raster assets.
- Continue logic-house cluster reviews after `logic-sorter-switch` and
  `logic-same-kind-detective`, prioritizing text-heavy or abstract clusters such
  as visual match, difference detective, block height maps, and three-view
  blocks.
