...
- 矩阵补格题要能从完整示例行推出答案：反馈必须先说第一行或完整行怎么变，再说第三行空格为什么补这个，不能只给抽象规则。3x3 `matrix` 是唯一答案视觉表面，不能同时挂 `sceneImage`；标准 1280x720 视口下，题干、矩阵、选项和题目控制必须完整同屏。
...
- 图形补一补的矩阵、选项和题目控制在 1280x720 下必须完整同屏；375px 宽手机可以纵向滚动，但矩阵格子、组合图形和选项不能横向溢出或互相遮挡。
