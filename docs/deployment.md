...
## Current local curriculum build

The local Mac milestone is generated by pnpm mac:install. The full exploration build includes 621 exploration activities plus the unchanged 489 enlightenment rounds. Required cue audio is local; source materials from Downloads are not packaged. See specs/029-curriculum-benchmark/verification/mac-install-full.txt and full-qa.md. Work remains on local dev; no remote release was requested.
