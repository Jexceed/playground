...
横屏尺寸（667×375、844×390、932×430）的静态布局检查。
`audit:douyin-minigame` 重新计算每个运行时文件的 SHA-256，并检查 AppID、
横屏、1280×720 设计分辨率、功能裁剪、14 MB 运行时资源预算和禁止资源。

## Cocos Creator 构建

1. 使用 Cocos Creator 3.8.8 打开 `doyingame/`，等待首次资源导入完成。
2. 打开 `assets/scenes/Main.scene`，先做 Web Mobile 预览。
3. 检查 667×375、844×390、932×430 三种横屏；各游戏抽查首题、中间题和
   末题，完整跑一次换游戏、跳题、选错、重试、答对、下一题。
4. 在构建发布面板选择“抖音小游戏”，确认横屏和 AppID
...
的空包；随后验证构建产物中的 AppID、横屏声明、主场景、语音、图片、启动钩子
和体积。开发者工具点过“编译”后，`pnpm test:douyin-minigame:devtools` 会读取
工具编译日志，要求当前构建得到成功结果，避免只打开项目却没有编译的假通过。
本次结果为 761 个文件、338 条 MP3、19 张 PNG、12.01 MB，输出目录为
...
| 项目 | 通过标准 | 当前状态 |
|---|---|---|
| 模拟器首次启动 | 5 秒内直接出现横屏三栏可作答工作台 | 已通过：iPhone 15 Pro 横屏重新编译后直接进入第 1 题；`Main.scene` 日志约 16.4 ms |
| 题目闭环 | 选错可重试，答对可下一题，语音不叠加 | 已通过答错→具体提示→改选→答对→家长追问→下一题；语音无运行错误，听感与叠音继续由真机确认 |
| 本地进度 | 退出重进恢复；损坏存档回到空进度 | 已通过：恢复到第 4 题；注入损坏值后安全回到第 1 题工作台 |
...
| 安全区 | 刘海和底部手势区不遮挡三栏或核心按钮 | 已通过 iPhone 15 Pro 与 Xiaomi 15 横屏；修复开发工具返回竖屏安全区坐标时的旋转换算；其余横屏设备待重新抽查 |
| 本地构建包 | AppID、主场景、横屏声明和资源闭包正确，未超过当前未分包 16 MB 门槛 | 已通过：12.01 MB，`deviceOrientation: landscape` |
...
- 2026-08-13 横屏同款改造：设计分辨率改为 1280×720，启动后直接恢复最近
  题目；页面与本机版统一为左侧品牌/八游戏、中间题面/横向选项、右侧题号/
  家长提示/成长记录。iPhone 15 Pro 模拟器实际编译并截图确认三栏、刘海和底部
  手势区可用。修复了开发工具在横屏窗口仍返回竖屏 safeArea 坐标而压缩半屏的
  问题，也固定了异步 Sprite 加载后改变预设尺寸的问题。
- Xiaomi 15 / Android 14 横屏也通过三栏与安全区抽查；此次抽查发现 Cocos 的
  ByteDance 转译会把 `Set` 展开编成嵌套集合，已把进度去重改为数组实现，并在
  源码检查和最终构建检查中同时禁止该转译产物。
...
- 人工闭环覆盖：第 3 题选错得到“从左到右，一个一个点着数，不要漏掉”的具体
  提示；改选 3 后得到正确反馈和家长追问，点击下一题进入 4/18；横屏版重启后
  直接恢复第 4 题。
...
- 旧竖屏版曾覆盖 iPhone SE 2、iPhone 15 Pro、Xiaomi 15、iPad；横屏版已经在
  iPhone 15 Pro 与 Xiaomi 15 完成开发工具实测，其余设备需按新的横屏矩阵再次
  确认，不能沿用旧版竖屏结论。
...
调用 `tt.navigateToScene({ scene: "sidebar" })`。入口放在右侧家长操作区，不
使用金币、奖励或诱导儿童复访。
